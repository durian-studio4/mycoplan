...
const TableTotalComponent: React.FC<Props> = ({ name }) => {
  ...
  useEffect(() => {
    const timeOut = setTimeout(() => {
      fetchList(
        `${REACT_APP_ENV}/admin/sales/total?category=${category}&start_date=${rangePickerValue[0]}&end_date=${rangePickerValue[1]}&merchant=${id}`,
      );
    }, 0);
    ...
    >
      <Row justify="space-between">
        ...
        <Button
          ...
          onClick={() =>
            onDownloadCSV({
              url: `${REACT_APP_ENV}/admin/sales/total?category=${category}&start_date=${rangePickerValue[0]}&end_date=${rangePickerValue[1]}&merchant=${id}&download=1`,
