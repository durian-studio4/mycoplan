...
const TableProdukComponent: React.FC<Props> = ({ name }) => {
  ...
  useEffect(() => {
    const timeOut = setTimeout(() => {
      fetchList(
        `${REACT_APP_ENV}/merchant/sales/products?category=${category}&start_date=${rangePickerValue[0]}&end_date=${rangePickerValue[1]}`,
      );
    }, 0);
  ...
  const columns = useMemo(
    () => [
      ...
      },
      {
        ...
        key: 'category_sub',
        ...getColumnSearchProps('category_sub'),
      },
      {
        align: 'center',
        title: 'Unit',
    ...
    >
      <Row justify="space-between">
        ...
        <Button
          ...
          onClick={() =>
            onDownloadCSV({
              url: `${REACT_APP_ENV}/merchant/sales/products?category=${category}&start_date=${rangePickerValue[0]}&end_date=${rangePickerValue[1]}&download=1`,
