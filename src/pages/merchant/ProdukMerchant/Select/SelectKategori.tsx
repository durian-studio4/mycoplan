...
const SelectKategoriComponent: React.FC<Props> = ({ initial, disabled, handleChange }) => {
  ...
  const fetching = async (param: string) => {
    ...
    try {
      const wait = await axios({
        method: 'get',
        baseURL: `${REACT_APP_ENV}/merchant/product/categories?category=1`,
  ...
  return (
    <Select
      labelInValue
      defaultValue={{ key: initial || 'Mohon Pilih Kategori' }}
      style={{
        width: '100%',
        minHeight: '2em',
      }}
