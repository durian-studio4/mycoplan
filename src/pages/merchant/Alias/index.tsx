...
const MerchantAliasComponent: React.FC<Props> = () => {
  ...
  useEffect(() => {
    const timeOut = setTimeout(() => {
      fetchUnit(`${REACT_APP_ENV}/admin/product-aliases`);
    }, 0);
  ...
  const createUnit = ({ json, clear }: Unit) => {
    postCreate(`${REACT_APP_ENV}/admin/product-aliases`, json, clear);
  };

  const updateUnit = ({ json }: any) => {
    postUpdate(`${REACT_APP_ENV}/admin/product-aliases/${id_update}`, json);
  };

  const deleteUnit = (id: string) => {
    postDelete(`${REACT_APP_ENV}/admin/product-aliases/${id}`);
  };
  ...
  return (
    <div>
      <p className={styles.title}>Alias Produk</p>
...
export default MerchantAliasComponent;
